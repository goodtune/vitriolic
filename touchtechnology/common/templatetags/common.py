...
camel_case_re = re.compile(
    r"(?P<new_word>((?<![A-Z])[A-Z]|[A-Z](?![A-Z0-9])))")
...
def _cached_perms_for_model(model, ttl=60):
    """
...
def twittify(s):
    ...
    return mark_safe(
        twitter_re.sub(r'@<a class="twitter user" '
                       'target="_blank" '
                       'href="http://twitter.com/\\2">\\2</a>', s))
...
class NavigationNode(Node):
    ...
    def render(self, context):
        r_kwargs = dict([(str(k), v.resolve(context, ignore_failures=True))
...
def do_navigation(root=None, start_at=None, stop_at=None, current_node=None,
    ...
    if root is not None:
        ...
        if current_node is not None and \
           current_node.tree_id == root.tree_id and \
           root.lft < current_node.lft and \
           not expand_all_nodes:
    ...
    hidden_from_navigation = create_exclude_filter(
        nodes_hidden_from_navigation)
...
@register.inclusion_tag('touchtechnology/common/templatetags/pagination.html',
                        takes_context=True)
...
def version(package, url=None):
    ...
    try:
        ...
        logger.exception('Attempting to lookup version for package "%s" not '
                         'in pkg_resources', package)
    if url:
        context['url'] = url
    return context
...
def check_permission(obj, checker):
    ...
    return [
        model_perm.codename
        for model_perm in _cached_perms_for_model(obj._meta.model)
...
def has_permission(obj, user):
    ...
    logger.debug('model="%s.%s", object_id="%s", user="%s"',
                 model._meta.app_label, model._meta.model_name,
                 getattr(obj, 'pk', ''), user.get_username())
    ...
    # What are the permissions that this model accepts?
    model_perms = _cached_perms_for_model(model, ttl=300)
    ...
    # if they transfer permission to the user.
        ...
        for group in user.groups.all():
            ...
            logger.debug('user="%s", group="%s", permissions="%s"',
