...
def v1_api_root(request, format=None):
    ...
    if apps.is_installed('touchtechnology.news'):
        try:
            endpoints['news'] = reverse('v1:news:api-root', request=request, format=format)
    ...
    if apps.is_installed('tournamentcontrol.competition'):
        try:
            endpoints['clubs'] = reverse('v1:competition:club-list', request=request, format=format)
            endpoints['competitions'] = reverse('v1:competition:competition-list', request=request, format=format)
        except NoReverseMatch:
